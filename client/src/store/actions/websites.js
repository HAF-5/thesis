...
export const setWebsite = () => async (dispatch, getState) => {
  try {
    ...
    let response = await fetch(
      `${process.env.REACT_APP_API}/api/website/${id}`
    );
...
export const deleteWebsite = id => async (dispatch, getState) => {
  try {
    let response = await fetch(
      `${process.env.REACT_APP_API}/api/website/delete/${id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json"
...
export const updateWebsiteDispatcher = payload => ({
  type: UPDATE_WEBSITE,
  payload
});
export const updateWebsite = data => async (dispatch, getState) => {
  try {
    let response = await fetch(
      `${process.env.REACT_APP_API}/api/website/update/${data.id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
    ...
    if (response.status === 200) {
      dispatch(updateWebsiteDispatcher(doc));
